...
trained at their own rate, the same architecture beats BERT-only by
$\TaskThreeSeparateVsBert$
...
$\TaskThreeSeedDelta \pm \TaskThreeSeedDeltaSd$ over five paired seeds
($p = \TaskThreeSeedP$), the largest effect we
measured --- and one that transfers: applied to the contrastive dual encoder it
doubles retrieval R@1 and lifts R@10 from $\TaskFourSharedCtwoaTen$ to
$\TaskFourSeparateCtwoaTen$. A six-listener
study corroborates those retrieval numbers from the other direction (mean
$\TaskFourHumanMean/5$) and adds what recall cannot see: the ranking within the model's own
...
$\TaskOneMusicCapsLexicalMicro$ where our model scores $\TaskOneMusicCapsMicro$ --- a margin small enough that, on
...
\item Complete implementations of all four tasks, with more than $140$ unit
...
\midrule
MagnaTagATune (metadata) & \TaskOneMtatMicro & \TaskOneMtatMacro \\
\quad B1 random-prevalence & \TaskOneMtatRandomMicro & \TaskOneMtatRandomMacro \\
\quad oracle ceiling & 0.673 & 0.575 \\
\midrule
MusicCaps (raw captions) & \textbf{\TaskOneMusicCapsMicro} & \TaskOneMusicCapsMacro \\
\quad B5 lexical match & \TaskOneMusicCapsLexicalMicro & \textbf{\TaskOneMusicCapsLexicalMacro} \\
\midrule
MusicCaps (stripped) & \TaskOneStrippedMicro & \TaskOneStrippedMacro \\
...
\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{../results/plots/attention_task1_mtat.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{../results/plots/training_curves.png}
...
\midrule
B4 PCA$+$MLP & \TaskTwoBFourParams & \TaskTwoBFourAcc & \TaskTwoBFourMacro \\
GraphSAGE & \TaskTwoSageParams & \TaskTwoSageAcc & \TaskTwoSageMacro \\
GAT & \textbf{\TaskTwoGatParams} & \TaskTwoGatAcc & \TaskTwoGatMacro \\
CNN baseline (B2) & \TaskTwoCnnParams & \textbf{\TaskTwoCnnAcc} & \textbf{\TaskTwoCnnMacro} \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
GraphSAGE & \TaskTwoSageAcc & \TaskTwoSageMacro & \TaskTwoChordSageAcc & \TaskTwoChordSageMacro \\
GAT & \TaskTwoGatAcc & \textbf{\TaskTwoGatMacro} & \TaskTwoChordGatAcc & \TaskTwoChordGatMacro \\
\midrule
CNN (B2) & \multicolumn{4}{c}{\TaskTwoCnnAcc\ acc / \textbf{\TaskTwoCnnMacro} Macro-F1} \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
BERT-only & 66.4M & \TaskThreeBertMacro & \TaskThreeBertMicro & \TaskThreeBertAuc \\
GNN-only & 31.8K & \TaskThreeGnnMacro & \TaskThreeGnnMicro & \TaskThreeGnnAuc \\
Early concat & 66.4M & \TaskThreeConcatMacro & \TaskThreeConcatMicro & \TaskThreeConcatAuc \\
Cross-attention & 66.8M & \TaskThreeCrossMacro & \TaskThreeCrossMicro & \TaskThreeCrossAuc \\
\midrule
\quad $+$ frozen BERT & 0.5M & 0.118 & 0.315 & 0.197 \\
\quad $+$ non-BERT lr $10^{-3}$ & 66.8M & \textbf{\TaskThreeSeparateMacro} & \textbf{\TaskThreeSeparateMicro} & \textbf{\TaskThreeSeparateAuc} \\
\bottomrule
\end{tabular}
\end{table}
...
architecture, no data and no seed --- raises Macro-F1 from $\TaskThreeCrossMacro$ to
$\mathbf{\TaskThreeSeparateMacro}$, Micro-F1 from $\TaskThreeCrossMicro$ to
$\mathbf{\TaskThreeSeparateMicro}$ and AUC-PR from $\TaskThreeCrossAuc$ to
$\mathbf{\TaskThreeSeparateAuc}$. The same run's attention entropy falls from
...
predicted. Against BERT-only this is $\TaskThreeSeparateVsBert$ Macro-F1:
...
\midrule
Macro-F1 & $\TaskThreeSeedSharedMacro\pm\TaskThreeSeedSharedMacroSd$ & $\TaskThreeSeedSeparateMacro\pm\TaskThreeSeedSeparateMacroSd$ & $\TaskThreeSeedDelta\pm\TaskThreeSeedDeltaSd$ & $\TaskThreeSeedP$ \\
Micro-F1 & $\TaskThreeSeedSharedMicro\pm\TaskThreeSeedSharedMicroSd$ & $\TaskThreeSeedSeparateMicro\pm\TaskThreeSeedSeparateMicroSd$ & $\TaskThreeSeedMicroDelta\pm\TaskThreeSeedMicroDeltaSd$ & $\TaskThreeSeedMicroP$ \\
AUC-PR   & $\TaskThreeSeedSharedAuc\pm\TaskThreeSeedSharedAucSd$ & $\TaskThreeSeedSeparateAuc\pm\TaskThreeSeedSeparateAucSd$ & $\TaskThreeSeedAucDelta\pm\TaskThreeSeedAucDeltaSd$ & $\TaskThreeSeedAucP$ \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
Shared lr & Caption $\to$ Audio & \TaskFourSharedCtwoaOne & \TaskFourSharedCtwoaFive & \TaskFourSharedCtwoaTen \\
          & Audio $\to$ Caption & \TaskFourSharedAtocOne & \TaskFourSharedAtocFive & \TaskFourSharedAtocTen \\
\midrule
Separate  & Caption $\to$ Audio & \textbf{\TaskFourSeparateCtwoaOne} & \textbf{\TaskFourSeparateCtwoaFive} & \textbf{\TaskFourSeparateCtwoaTen} \\
graph lr  & Audio $\to$ Caption & \TaskFourSeparateAtocOne & \TaskFourSeparateAtocFive & \TaskFourSeparateAtocTen \\
\midrule
Random    & ---                 & \TaskFourRandomOne & --- & \TaskFourRandomTen \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{../results/plots/task4_scale.png}
...
\midrule
Task 3, supervised fusion & \textbf{\TaskFourSupervisedMicro} & \textbf{\TaskFourSupervisedMacro} & \textbf{\TaskFourSupervisedAuc} \\
Task 4, zero-shot & \TaskFourZeroMicro & \TaskFourZeroMacro & \TaskFourZeroAuc \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[H]
\centering
\includegraphics[width=\linewidth]{../results/plots/tsne_task3_views.png}
...
project repository. Headline tables and claims are generated from
\texttt{results/metrics.json} into \texttt{report/generated\_metrics.tex} by
\texttt{src/make\_report\_tex.py}; the report includes figures directly from
\texttt{results/plots/}. Missing required values stop generation, preventing
stale hand-copied tables or plots from compiling unnoticed.
