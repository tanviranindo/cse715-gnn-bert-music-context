...
(entropy $0.695$, peak $4.19\times$) and lifts Macro-F1 from $0.192$ to
$0.246$, the largest single effect we measured. We report every number against
...
We state two caveats plainly. First, these are single-seed runs; the
learning-rate result is large enough ($+0.109$ Micro-F1) to be unlikely to be
seed noise, but we have not measured its variance. Second, the earlier draft's
...
indicator of a working fusion. The natural next step is a seed sweep over the
separate-learning-rate configuration, which we report from a single seed.
