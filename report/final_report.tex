...
of the between-ablation margins in Table~\ref{tab:t3ablation}, which is a caution against reading
...
\noindent \textbf{Graph paths.} Each case study also records the heaviest walk out of its busiest segment, so the route the similarity graph ties together can be read against the caption rather than inferred from a degree sequence. On MusicCaps those walks are two nodes long, because a $10$\,s clip at $1.5$\,s segments yields about six nodes at average degree $1.7$. That is not a defect of the extraction but a fact about the corpus, and it is part of why the graph branch contributes little here: there is very little structure to traverse.
