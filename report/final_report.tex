...
doubles retrieval R@1 and lifts R@10 from $0.0220$ to $0.0339$. We report every number against
...
\item Complete implementations of all four tasks, with $112$ unit
...
($R^2 = 0.914$), and MusicCaps' official split is two orders of magnitude short
...
\noindent \textbf{The Task 3 finding transfers.} Giving the graph tower its own
...
official split provides. We report the extrapolation to characterise the
regime, not as a forecast: it spans an order of magnitude beyond our data.
...
\textbf{The evaluation we did not run.} The specification asks for a human
...
claims rest entirely on R@K against a $2{,}773$-clip gallery. The instrument is
complete: \texttt{src/human\_eval.py} builds a self-contained rating sheet that
embeds all $30$ retrieved clips as playable YouTube segments --- each cued to
the exact ten-second window its caption describes, which matters because those
windows sit up to $6{:}50$ into their source videos --- and aggregates returned
ratings with per-rank means, refusing to score fewer than five raters.
What is missing is only the listeners. We record this as a gap against the
brief rather than a considered omission.
...
standard split, a measured attention collapse, and --- after correcting our own
checkpoint policy --- a multi-task term that turned out to be roughly free
rather than costly. Each of these would have been invisible had we reported only
...
runs on CPU in seconds from the committed artifacts.
