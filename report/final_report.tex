...
\begin{tabular}{lccc}
\toprule
& Shared lr & Non-BERT lr & Paired $\Delta$ \\
\midrule
...
supervised model. Doing that fairly requires both to be scored on the same
clips with the same label set, so we retrained the Task~3 cross-attention
fusion on MusicCaps under the \emph{official AudioSet eval partition} --- the
same $2{,}195/387/2{,}773$ split the contrastive model uses --- over the same 50
tags, derived from the training split alone.

\begin{table}[H]
\centering\small
\caption{Zero-shot versus supervised on identical data: same corpus, same
official eval split, same 50 tags derived from training clips only. Neither the
zero-shot vocabulary nor its decision threshold sees a test label.}
\label{tab:zeroshot}
\begin{tabular}{lccc}
\toprule
& Micro-F1 & Macro-F1 & AUC-PR \\
\midrule
Task 3, supervised fusion & \textbf{0.657} & \textbf{0.593} & \textbf{0.620} \\
Task 4, zero-shot & 0.100 & 0.093 & 0.058 \\
\bottomrule
\end{tabular}
\end{table}

\noindent Contrastive alignment alone recovers about $15\%$ of supervised
Micro-F1. That is a smaller figure than our earlier draft reported, for two
reasons we record rather than quietly correct. First, that draft compared
against the Task~1 model, not Task~3 as the specification asks. Second, and more
seriously, its zero-shot protocol leaked: the tag vocabulary was chosen by
frequency \emph{in the test split} and the decision threshold was tuned on the
test labels. The encoder never saw tag supervision, but the evaluation did, and
both choices flatter the score. Fixing them moves Micro-F1 from $0.107$ to
$0.100$; the correction is small, but a leak measured after the fact is worth
less than a protocol that could not leak.
