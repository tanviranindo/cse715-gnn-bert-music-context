...
that giving the graph branch its own learning rate un-collapses that attention
(entropy $0.695$, peak $4.19\times$) and lifts Macro-F1 by
$+0.067 \pm 0.023$ over five paired seeds ($p = 0.003$), the largest effect we
measured. We report every number against
...
\item Complete implementations of all four tasks, with $92$ unit
...
\midrule
B4 PCA$+$MLP (audio features) & 8,712 & 0.379 & 0.371 \\
...
\quad $+$ separate GNN lr $10^{-3}$ & 66.8M & \textbf{0.246} & \textbf{0.415} & \textbf{0.248} \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{The second cause is the operative one.} Giving the graph branch, the
fusion block and the heads their own learning rate of $10^{-3}$ --- changing no
...
on this corpus, structural fusion helps once the graph branch is actually
trained.
...
\begin{tabular}{lccc}
\toprule
& Shared lr & Separate GNN lr & Paired $\Delta$ \\
\midrule
Macro-F1 & $0.172 \pm 0.023$ & $0.240 \pm 0.020$ & $+0.067 \pm 0.023$ ($p{=}0.003$) \\
Micro-F1 & $0.335 \pm 0.024$ & $0.416 \pm 0.028$ & $+0.081 \pm 0.018$ ($p{=}0.001$) \\
AUC-PR   & $0.181 \pm 0.015$ & $0.260 \pm 0.018$ & $+0.079 \pm 0.010$ ($p{<}0.001$) \\
\bottomrule
\end{tabular}
\end{table}
...
that seed-to-seed variation ($\pm0.023$ Macro-F1) is itself larger than several
of the between-ablation margins in Table~3, which is a caution against reading
those smaller differences too firmly. We also reproduced seed 42 on different
hardware and obtained $0.254$ rather than $0.246$, so the third decimal is not
...
MagnaTagATune training data is identical across both runs. \textbf{These two
columns are from our earlier run and were evaluated at a fixed final epoch, not
at the best validation checkpoint used everywhere else in this report}; the
DEAM audio host became unreachable before we could re-run them, so they are not
directly comparable with Table~3 and are reported as indicative only.}
\begin{tabular}{lccc}
\toprule
...
We flag one reproducibility gap honestly. DEAM's official host
(\texttt{cvml.unige.ch}) stopped responding during this work, and the only
surviving audio mirror we located does not ship the \texttt{metadata.zip} whose
genre/artist/title fields we use to build DEAM's pseudo-captions. We could have
re-run this experiment with empty captions, but that would silently change the
setup rather than reproduce it, so we left the original numbers in place and
labelled them. Re-running this row under the corrected checkpoint policy is the
one outstanding item in this report.
...
\midrule
Caption $\to$ Audio & 0.0029 & 0.0108 & 0.0220 \\
...
\noindent Retrieval is $8.0\times$ random at R@1 and $6.1\times$ at R@10, with
median rank $527$ of $2{,}773$ --- that is, the median true clip still sits in
the top $19\%$ of the gallery. The alignment is real but weak, and it is
weaker than our random-split measurement implied for two compounding reasons:
the gallery is $5.2\times$ larger, and the official partition leaves only
$2{,}195$ training pairs against the $4{,}284$ a random $80/10/10$ split
provided. Across our ten saved qualitative examples, \textbf{none} places the
true clip at rank 1.
...
gives Micro-F1 $0.107$, against $0.488$ for the supervised stripped-caption
Task 1 model on the same corpus and the same official split. Contrastive
alignment alone thus recovers roughly $22\%$ of supervised performance.
...
\texttt{librosa.chroma\_stft} $\to$ \texttt{estimate\_tuning} $\to$
...
\textbf{Two evaluations we did not run.} The specification asks for a human
study on Task 4 --- at least five listeners rating retrieved clips $1$--$5$ for
whether the clip matches the query caption. We did not run it, so our retrieval
claims rest entirely on R@K against a $2{,}773$-clip gallery; with $R@10$ at
$0.022$ a human study would mostly be measuring near-misses, but it remains a
gap against the brief rather than a considered omission. Separately, the DEAM
multi-task numbers in Table~6 were scored at the final epoch, before we adopted
best-validation checkpointing everywhere else; the metadata archive that
supplies DEAM's pseudo-captions is no longer retrievable from any surviving
mirror, so that run could not be repeated under the corrected policy. Both
comparisons in that table share the methodology, so the reported delta stands
even though the absolute values are not directly comparable with Table~4.
...
Code, seeded configurations, per-run metrics, $155$ example graphs, the t-SNE
